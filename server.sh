...
if [ -z $2 ]; then
  echo "Usage: $0 <client CN> <client SAN>"
  exit 1
fi

# autoincrement serial
SERIAL=$((`cat serial` + 1))
#HEXSERIAL=`printf "%02X\n" ${SERIAL}`
...
# create client CSR using SAN
openssl req -new -sha256 -key ${1}-NOPASS.key -reqexts SAN -config <(cat /etc/ssl/openssl.cnf <(printf "[SAN]\nsubjectAltName=${2}")) -out ${1}.csr

# self-sign
openssl x509 -req -days 365 -extfile <(printf "subjectAltName=${2}") -in ${1}.csr -CA ca.crt -CAkey ca.key -set_serial ${SERIAL} -out ${1}.crt && echo ${SERIAL} > serial
